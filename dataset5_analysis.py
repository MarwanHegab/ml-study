...
def evaluate_nn(X, y, hyperparams, splits=10):
    ...
    # convert from string labels to one hot
    le = LabelEncoder()
    y_int = le.fit_transform(y)
    y_oh = np.zeros((y_int.size, num_classes))
    y_oh[np.arange(y_int.size), y_int] = 1
...
def plot_results(dt_res, rf_res, nn_res, nn_hyperparams):
    ...
    plt.ylabel('Accuracy')
    plt.savefig('figures/dataset5_dt_acc.png')
    ...
    plt.ylabel('Accuracy')
    plt.savefig('figures/dataset5_rf_acc.png')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.savefig('figures/dataset5_nn_acc.png')
...
def generate_nn_learning_curve(X, y, best_hp):
    ...
    plt.ylabel('Cost J')
    plt.savefig('figures/dataset5_nn_cost.png')

if __name__ == '__main__':
    df = pd.read_csv('datasets/music_genres.csv')
    ...
    nn_hps = [
        {'epochs': 50, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32},
        {'epochs': 100, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32},
        {'epochs': 250, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32},
        {'epochs': 500, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32},
        {'epochs': 1000, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32},
        {'epochs': 2000, 'LN': [40, 40, 10], 'alpha': 0.1, 'lambda_reg': 0.01, 'mb_size': 32}
    ]
